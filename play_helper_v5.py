...
class PlayHelper(QtGui.QDialog):
    ...
    def run(self, title='playHelper'):
        ...
        self.setWindowIcon(self.icons['vedio'])
    ...
    def setStyles(self):
        ...
        # self.setStyleSheet("QDialog {background-image: url(tree.jpg);}")
        self.setStyleSheet("QWidget {background-color: #E5F9BD;}"
                           "QDialog {background-color: #FDFFDF;}"
                           "QToolBox {background-color: #BDF1F6;}"
                           "QPushButton {background-color: #A0E418;}"
                           # "QPushButton {background-color: #9FFBFB;}"
                           )

    def setLocations(self):
        w = self.app.desktop().width()
        h = self.app.desktop().height() # pos()
        # print(w, h)
        self.downloader.setGeometry(w-650, h-500, 280, 100)
        self.setGeometry(w-650, h-500, 600, 450)
        self.log.setGeometry(w-650, h-500, 400, 300)
    ...
    def createMainLayout(self):
        ...
        self.search = Search()
        self.search.text = ''
        self.search.dataReady.connect(self.searchDisply)
    ...
    def addSearchItemForToolbox(self, toolbox):
        ...
        goButton.clicked.connect(self.searchHandle)
        goButton.setFocus()
    ...
    def searchHandle(self):
        # 以下代码放这里,搜索栏有明显的闪动,放searchDisply基本没感觉
        # 视需求而定
        # try:
        ...
        self._search(t) # list_ -> [(text, url, icon) ...]
        
    def searchDisply(self, list_):
        try:
            self.search_table.hide()
            self.serach_layout.removeWidget(self.search_table)
        except Exception as e:
    ...
    def download(self):
        ...
        try:
            ts = chain(self.tables, [self.search_table])
            # ts = list(self.tables, self.search_table)
        except Exception as e:
            print(e)
            ts = self.tables
    ...
    def getInfoFromYouget(self, url):
        ...
        p = subprocess.Popen(['you-get', '-i', url], shell=True,
...
class Search(QtCore.QThread):
    ...
    def run(self):
        try: 
            self.dataReady.emit(Search.search(self.text))
        except:
            self.log('without ** self.text **')
...
class Downloader(QtGui.QWidget):
    ...
    def initUI(self): 
        ...
        self.setGeometry(500, 500, 280, 100)
    ...
    def popMessage(self, text='未选择任务或全部所选任务已经下载过'):
        '''
    ...
    def addMenuToBtn(self, btn, name):
        '''
        根据name找到进程并杀死
        '''
        m = QtGui.QMenu()
        a = QtGui.QAction("取消", self, triggered=partial(self.kill, name))
        m.addAction(a)
        # m.addSeparator()
        btn.setMenu(m)
        # self.action = a  # 保留action供以後使用 , !!! 不能这么保留
    
    def kill(self, name):
        '''
        todo: 其它模块与终止的耦合
        '''
        proc = self.tasks[name].process
        proc.terminate()
        # proc.kill()
        # self.tasksNum -= 1 # !!! 不能在此处使用, stream_watcher中已经有把关
        btn = self.tasks[name].btn
    ...
    def timerEvent(self, e):
        ...
        for t in self.tasks.values():
            try:
                ...
                if t.step == 100:
                    t.btn.setText('完成！')
                    t.btn.menu().deleteLater()
            except:
                pass # the ui was deleted 
                # 或者在删除旧ui界面中去除self.tasks中的旧任务

        # debug: print('任务数：', self.tasksNum)
        if self.tasksNum <= 0:
            self.timer.stop()
        # timer 不停止?
